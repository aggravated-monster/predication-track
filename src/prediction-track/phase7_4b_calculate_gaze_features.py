...
def get_output_file_name(suffix, prefix):
    return get_output_folder(suffix) + prefix + '_gaze_features.csv'

def get_summary_file_name(suffix):
    return get_output_base() + suffix + '_aggregated_gaze_features.csv'

def expand(input_file_path, suffix, prefix):

    print("Adding gaze features for: " + prefix)

    # load the dataframe
    df_in = pd.read_csv(input_file_path)

    # calculate delta_eye_x
    # to do this, add a column with a +1 offset
    df_in['previous_eye_x'] = df_in['eye_x'].shift(1)
    df_in['previous_eye_y'] = df_in['eye_y'].shift(1)
    # calculate delta-x
    df_in['delta_eye_x'] = df_in['previous_eye_x'] - df_in['eye_x']
    # calculate delta-y
    df_in['delta_eye_y'] = df_in['previous_eye_y'] - df_in['eye_y']

    # calculate euclidean distance
    df_in['delta_eye_euclid'] = df_in.apply(lambda x: tsimath.calculate_euclidean_distance(x['eye_x'],x['eye_y'],x['previous_eye_x'],x['previous_eye_y']), axis=1, result_type ='expand')

    # calculate abs of delta-x
    df_in['abs_delta_eye_x'] = df_in['delta_eye_x'].abs()
    # calculate abs of delta-y
    df_in['abs_delta_eye_y'] = df_in['delta_eye_y'].abs()

    print(df_in)

    # take only the necessary columns
    df_in = df_in[['timestamp','subject','name','delta_eye_x','abs_delta_eye_x','delta_eye_y','abs_delta_eye_y','delta_eye_euclid']]
...
if __name__ == "__main__":
    ...
    print('*** Calculating gaze features completed ***')
